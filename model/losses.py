        ...
        for i in range(1, s):
            for j in range(b):
                if selector is not None and selector[j][1] > 0.5:
                    loss_seg, p = self.bce(data['logits_%d'%i][j:j+1], data['cls_gt'][j:j+1,i], it)
                    loss_score = self.score_loss(data['mask_score_%d' % i][j:j+1], data['gt_score_%d' % i][j:j+1])
                else:
                    loss_seg, p = self.bce(data['logits_%d'%i][j:j+1,:2], data['cls_gt'][j:j+1,i], it)
                    loss_score = self.score_loss(data['mask_score_%d' % i][j:j+1], data['gt_score_%d' % i][j:j+1])

                losses['loss_%d'%i] = losses['loss_%d'%i] + loss_seg / b + loss_score / b
                losses['p'] += p / b / (s-1)
