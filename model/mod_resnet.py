...
def resnet50(pretrained=True, extra_chan=0):
    ...
    if pretrained:
        load_weights_sequential(model, model_zoo.load_url(model_urls['resnet50']), extra_chan)  #download
        # load_weights_sequential(model, torch.load('path_to_yours'), extra_chan)       #load yours
    return model
